...
function SupervisorApp() {
  ...
  const [editableScenarios, _setEditableScenarios] = useState<Record<string, { scenario: RealtimeAgent[], companyName: string, displayName: string }>>(() => {
  ...
  const [editableMetaprompt, _setEditableMetaprompt] = useState<string>(() => {
    if (typeof window !== 'undefined') {
...
<TOOL_DESCRIPTIONS>`;
  ...
  const [isEventsPaneExpanded, _setIsEventsPaneExpanded] = useState<boolean>(() => {
